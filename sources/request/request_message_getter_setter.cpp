...
RequestState RequestMessage::GetState() const { return (state_); }

const std::string &RequestMessage::GetHeaderValue(
...
bool RequestMessage::IsLastChunk() const { return (last_chunk_flag_); }

int RequestMessage::GetChunkSize() const { return (chunk_size_); }
...
std::string &RequestMessage::GetUri() { return (uri_); }
...
std::string &RequestMessage::GetBody() { return (body_); }

const std::string &RequestMessage::GetResolvedUri() const {
	return resolved_uri_;
}

const std::string &RequestMessage::GetQuery() const { return query_; }
...
void RequestMessage::SetState(RequestState code) { state_ = code; }

void RequestMessage::SetChunked(bool is_chunked) { is_chunked_ = is_chunked; }

void RequestMessage::SetContentSize(int size) { content_size_ = size; }
...
void RequestMessage::SetChunkSize(size_t size) { chunk_size_ = size; }

void RequestMessage::SetResolvedUri(const std::string &resolved_uri) {
	resolved_uri_ = resolved_uri;
}

void RequestMessage::ClearChunkSize() { chunk_size_ = 0; }

void RequestMessage::ClearChunkSizeStr() { chunk_size_str_ = ""; }

void RequestMessage::ClaerChunkBody() { chunk_body_ = ""; }

/* TEST 전용 */
void RequestMessage::setUri(const std::string &uri) { uri_ = uri; }

void RequestMessage::SetQuery(const std::string &query) { query_ = query; }
