	...
	while (req_msg.GetState() != DONE && size) {
		switch (req_msg.GetState()) {
			...
			case BODY_CHUNK_EXTENSION:
				req_msg.SetState(ChunkExtension(*input));
				break;
			case BODY_CHUNK_EXTENSION_NAME:
				req_msg.SetState(ChunkExtensionName(*input));
				break;
			case BODY_CHUNK_EXTENSION_VALUE:
				req_msg.SetState(ChunkExtensionValue(*input));
				break;
			...
			case BODY_CHUNK_TRAILER:
				req_msg.SetState(ChunkTrailer(*input));
				break;
			case BODY_CHUNK_TRAILER_NAME:
				req_msg.SetState(ChunkTrailerName(*input));
				break;
			case BODY_CHUNK_TRAILER_VALUE:
				req_msg.SetState(ChunkTrailerValue(*input));
				break;
			case BODY_CHUNK_TRAILER_CRLF:
				req_msg.SetState(ChunkTrailerCRLF(*input));
				break;
			case BODY_CHUNK_EMPTYLINE:
				req_msg.SetState(ChunkEmptyLine(*input));
				break;
			default:
...
static RequestState ChunkSizeCRLF(RequestMessage &req_msg, char c) {
	if (c != LF)
		throw HttpException(BAD_REQUEST,
							"(chunked parser) : syntax error. bare CR not "
							"allowed in chunk size line");
	else {
		int chunk_size = hexstrToDec(req_msg.GetChunkSizeStr());
		if (chunk_size == 0)
			return BODY_CHUNK_LASTDATA;	 // -> 0 혹은 extension 이후에 CRLF를
										 // 만나면 LASTDATA로 간다.
		if (chunk_size + req_msg.GetChunkSize() >
			req_msg.GetClientMaxBodySize()) {
			std::string err_msg =
				"(chunked parser) : max client body size is " +
...
static RequestState ChunkExtension(char c) {
	if (c == SP || c == HT)
		...
		throw HttpException(BAD_REQUEST, "(chunked parser) : syntax error. ");
}
...
static RequestState ChunkData(RequestMessage &req_msg, char c) {
	...
	if (req_msg.GetChunkSize() == 0) {
		...
		else  // RFC에 사이즈와 CLRF가 다르면 어떨 지 안나와있다. 일단
			  // 에러처리함.
			throw HttpException(BAD_REQUEST,
								"(chunked parser) : chunk size differs to "
								"actual chunk body size");
	} else if (0 <= c && c <= 127) {
...
static RequestState ChunkCRLF(RequestMessage &req_msg, char c) {
	if (c == LF) {
		...
		// std::cout << C_YELLOW << "new size: " << req_msg.GetBody().size() <<
		// std::endl;
		return BODY_CHUNK_START;
	} else
		throw HttpException(BAD_REQUEST,
							"(chunked parser) : syntax error. bare CR not "
							"allowd in chunk body");
}
...
static RequestState ChunkTrailer(char c) {
	if (c == CR)
		...
		throw HttpException(BAD_REQUEST,
							make_err_msg("(chunked parser) : syntax error. "
										 "invalid char for chunk trailer",
										 c));
}
...
static RequestState ChunkTrailerName(char c) {
	if (c == ':')
		...
		throw HttpException(BAD_REQUEST,
							make_err_msg("(chunked parser) : syntax error. "
										 "invalid char for chunk trailer name",
										 c));
}
...
static RequestState ChunkTrailerValue(char c) {
	if (c == SP || c == HT || isVChar(c))
		...
		throw HttpException(BAD_REQUEST,
							make_err_msg("(chunked parser) : syntax error. "
										 "invalid char for chunk trailer value",
										 c));
}
